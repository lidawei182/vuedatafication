      ...
      <div class="datav-container-left">
        ...
        <div id="alineids" class="datav-container-left-top" style="margin-top: 1%;"
          ...
          <dv-border-box-6 style="width:100%;height:100%;" :color="['#0931b5', '#7c95e8']">
            ...
            <LineDoughnut 
              :set-interval-get="set_interval" 
              :style-flatcake="{}" 
              :chart-pie-data="callStatistics"
              :click-ddvaluf = "clickDdvaluf"
              class-name="alineid" 
              class-names="alineids" />
          </dv-border-box-6>
        </div>
...
//const wsurl = "ws://www.mmcall.test.net:7003";
//const wsurl = "ws://192.168.0.157:7003"
//const wsurl = "ws://www.mmcall.net:7004";
//const wsurl = "ws://172.31.46.124:7003";
//172.31.46.124
//172.31.47.253:7003
  ...
  methods: {
    _WebSocketphp() {
      ...
      _this.ws.onopen = function() {
        ...
        let datalogin = {};
        datalogin.type = "login";
        datalogin.room_id = _this.jdno;
        datalogin.client_name = _this.group;
        _this.ws.send(JSON.stringify(datalogin));

        let datapush = {};
        datapush.type = "push";
        datapush.room_id = _this.jdno;
        datapush.client_name = _this.group;
        _this.ws.send(JSON.stringify(datapush));
      };
      ...
      _this.ws.onmessage = function(e) {
        ...
        if(obj.type == "push") {
          if(obj.status === 'ok'){
            _this.calllist = obj.list;
            _this.calllistTotal = obj.total[0].total;
          }else{
            _this.calllist = [];
            _this.calllistTotal = 0;
          }
        }
      };
    },
    ...
    iniIntegration() {
      ...
      this.dateValue = this.dashboardDateValue;
    ...
    optionschange(e) {
      ...
      this.dateValue = e;
      this.clickDdvaluf = e;
      this.$store.commit('SET_DDVALUE', this.dateValue);
      let themoonarraylist = JSON.stringify(this.themoonarraylist);
      if (this.dateValue == '月') {
        this.themoon();
        themoonarraylist = JSON.stringify(this.themoonarraylist);
    ...
    setarrayResidence(data) {
      ...
      for (let i = 0; data.length > i; i++) {
        ...
        if (typeof (data[i].identity_card) == 'string') {
          data[i].sex = this.analyzeIDCard(data[i].identity_card).sexCode;
          data[i].age = this.GetAge(data[i].identity_card);
          
        }

        if (data[i].sex == '女') {
          arraysex1.push(data[i]);
          agesex1.push(data[i].age);
        } else {
          arraysex2.push(data[i]);
          agesex2.push(data[i].age);
        }
      ...
      let data_oldman_type = [
        { name: "离休", value: array_oldman_type1.length ? array_oldman_type1.length : 0 },
        { name: "退休", value: array_oldman_type2.length ? array_oldman_type2.length : 0 },
        { name: "特困", value: array_oldman_type3.length ? array_oldman_type3.length : 0 },
        { name: "低保", value: array_oldman_type4.length ? array_oldman_type4.length : 0 },
