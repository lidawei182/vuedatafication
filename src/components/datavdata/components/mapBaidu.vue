  ...
  <div class="map-baidu">
    <div class="set-up" @click="setupMapPositioning">
      ...
      <div class="positioning-map" v-show="positioningMap">
        ...
        <span class="parallelogram" @click="submitPositioningupdata">取消</span>
      </div>
      <dv-border-box-5
        :color="['#7c95e8', '#345ffc']" 
    ...
    <div class="map-top-const map-top-const-box">
      <span style="color: #22eb47;">在线设备数{{ isonlinemaphave }}&nbsp;&nbsp;</span>
      <span style="color: #aaa9a9;">离线设备数{{ isonlinemapnot }}&nbsp;&nbsp;</span>
      <span>总设备数{{ constmap }}</span>
    </div>
...
export default { 
  ...
  methods: { 
    ...
    async getMapIntegration(){
      let that = this;
      that.responseData = await that.mapIntegrationdata();
      that.markerArr = that.responseData;
      that.constmap = that.markerArr.length;
      that.getmarkerArr(that.markerArr);
    },
    async mapIntegrationdata(){
      let that = this;
      let arrayresponseData = [];
      return new Promise((resolve) => {
        mapIntegration(that.code).then(response => {
          if(response.status == 1){
            let data = response.data.tree;
            for(let i = 0; data.length > i; i++){
              if( data[i].longitude1 != null){
                let datamap =  {
                ...
                arrayresponseData.push(datamap);
              }
            }
            resolve(arrayresponseData);
          }
        })
      })
    },
...
.anchorBL{
  ...
  .map-top-const-box {
    ...
    --border-size: 2px;
    --border-angle: 0turn;
    background-image: conic-gradient(from var(--border-angle), #213, #112 50%, #213), conic-gradient(from var(--border-angle), transparent 20%, #08f, rgb(140, 184, 250));
